...
export const createOrder = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (!address.location?.coordinates || address.location.coordinates.length < 2) {
    return res.status(400).json({
      message: "Address location is invalid",
    });
  }
  ...
  const orderItems = cartItems
    ...
    .filter((item): item is { itemId: string; name: string; price: number; quauntity: number } => item !== null);
...
// Internal endpoint called by utils service after payment verification
// Replaces the RabbitMQ consumer so the flow works without RabbitMQ
export const markOrderPaid = TryCatch(async (req, res) => {
  if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
  ...
  if (!order) {
    // Either already paid or not found — treat as success (idempotent)
    return res.json({ message: "Order already processed" });
  }

  console.log(`✅ Order paid via ${provider}: ${order._id} (paymentId: ${paymentId})`);

  // Notify the restaurant via socket
  try {
    await axios.post(
  ...
  res.json({ message: "Order marked as paid successfully", orderId: order._id });
});
...
export const updateOrderStatus = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    // Notify nearby riders directly via rider service (no RabbitMQ needed)
    if (status === "ready_for_rider") {
      try {
        await axios.post(
          `${process.env.RIDER_SERVICE}/api/rider/internal/notify-riders`,
          {
        ...
        console.error("Rider notification failed (non-fatal):", riderErr?.response?.data || riderErr?.message);
      }
    }
