...
const Restaurant = () => {
  ...
  return (
    <div style={{ background: "var(--color-bg-secondary)", minHeight: "100vh" }}>
      {/* Seller header banner – clean white design */}
      <div style={{ background: "linear-gradient(135deg, #fff8f8 0%, #ffffff 50%, #fdf4ff 100%)", borderBottom: "1px solid var(--color-border-light)", padding: "20px 0 0" }}>
        <div className="container">
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
            <div style={{ width: "40px", height: "40px", borderRadius: "var(--radius-md)", background: "var(--color-primary-light)", border: "1px solid rgba(226,55,68,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div>
              <p style={{ color: "var(--color-text-muted)", fontSize: "0.8125rem", margin: 0, fontWeight: 500 }}>Seller Dashboard</p>
              <h1 style={{ color: "var(--color-dark)", fontWeight: 800, fontSize: "1.25rem", margin: 0 }}>{restaurant.name}</h1>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background: "#fff", borderRadius: "var(--radius-xl)", boxShadow: "var(--shadow-card)", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid var(--color-border)" }}>
            {TABS.map(({ key, label, icon }) => (
              ...
              >
                <span style={{ color: tab === key ? "var(--color-primary)" : "var(--color-text-muted)" }}>{icon}</span>
